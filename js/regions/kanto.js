...
export async function loadLocations() {
  debug.debugLog("loadLocations kalt for Kanto");
  const res = await fetch(`https://pokeapi.co/api/v2/region/${regionData.kanto.id}`);
  if (!res.ok) {
  ...
  return `
    <div class="icon-entry">
      <label>
        ...
        <input type="checkbox" ${checked} onchange="window.kantoToggleCaught('${name}', this)">
      </label><br>
      <img src="${iconUrl}" alt="${name}" title="${capitalize(name)}" width="128" height="128" onclick="window.kantoShowLocations('${name}')">
    </div>
  `;
}
...
export async function showRegionalDex() {
  ...
  try {
    const dexUrl = regionData.kanto.pokedexUrl;
...
async function renderDex(output, titleRegion) {
  ...
  output.innerHTML = `
    ...
    <input type="text" id="search" placeholder="Søk etter Pokémon..." oninput="window.kantoFilterDex()">
...
window.kantoToggleCaught = toggleCaught;
window.kantoShowLocations = showLocations;
window.kantoFilterDex = filterDex;
