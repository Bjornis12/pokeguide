...
export async function loadLocations() {
  ...
  const res = await fetch(`https://pokeapi.co/api/v2/region/${regionData.hoenn.id}`);
  if (!res.ok) {
  ...
  return `
    <div class="icon-entry">
      <label>
        ...
        <input type="checkbox" ${checked} onchange="window.hoennToggleCaught('${name}', this)">
      </label><br>
      <img src="${iconUrl}" alt="${name}" title="${capitalize(name)}" width="128" height="128" onclick="window.hoennShowLocations('${name}')">
    </div>
  `;
}
...
export async function showRegionalDex() {
  ...
  try {
    const dexUrl = regionData.hoenn.pokedexUrl;
...
async function renderDex(output, titleRegion) {
  ...
  output.innerHTML = `
    ...
    <input type="text" id="search" placeholder="Søk etter Pokémon..." oninput="window.hoennFilterDex()">
...
window.hoennToggleCaught = toggleCaught;
window.hoennShowLocations = showLocations;
window.hoennFilterDex = filterDex;
